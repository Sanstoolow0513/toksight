import { weekdayIndex } from './period.js';

export const DEFAULT_LOCALE = 'zh-CN';
export const LOCALES = ['zh-CN', 'en'];

// Monday-first, matching the calendar grids.
export const WEEKDAYS = {
  'zh-CN': ['一', '二', '三', '四', '五', '六', '日'],
  en: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
};

export const MONTHS = {
  'zh-CN': ['1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月'],
  en: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
};

const MONTHS_LONG_EN = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];

// Exported for the key-parity test (test/i18n.test.js): zh-CN and en must
// carry exactly the same key set, or t()'s fallback would silently show
// Chinese strings to EN users (and vice versa).
export const tables = {
  'zh-CN': {
    docTitle: 'toksight · Token 用量与成本报告',
    toolbarAria: '报告设置',
    modeGroup: '统计周期',
    modeMonth: '月',
    modeYear: '年',
    prev: '上一期',
    next: '下一期',
    themeGroup: '配色',
    themeLight: '浅色',
    themeDark: '深色',
    themeSystem: '跟随系统',
    langGroup: '界面语言',
    refresh: '刷新数据库',
    refreshing: '正在刷新数据库',
    refreshFailed: '刷新数据库失败：{error}',
    reportActions: '报告操作',
    databaseActions: '全部用量数据',
    importDatabase: '导入数据库',
    exportDatabase: '导出数据库',
    importingDatabase: '正在合并…',
    exportingDatabase: '正在备份…',
    databaseMergeHint: '合并去重 · 保留现有数据',
    databaseAllHint: '所有 Agent · 所有日期',
    databaseImportDescription: '将备份中的用量合并到当前数据库，自动跳过重复记录。现有数据与已知费用优先保留，导入的历史在刷新后仍会保留。',
    databaseContents: '包含全部日期的用量、会话标题与目录、Cursor 导入记录和价格信息。仅支持 toksight 数据库，最大 256 MB。',
    databaseCancel: '取消',
    databaseMerge: '合并导入',
    databaseImportSuccess: '已合并：新增 {imported} 条、补全费用 {updated} 条、重复 {duplicates} 条。',
    databaseImportFailed: '数据库导入失败：{error}',
    databaseExportSuccess: '完整数据库备份已开始下载',
    databaseExportFailed: '数据库导出失败：{error}',
    databaseInvalid: '文件不是有效的 toksight 数据库，或数据库版本暂不支持。',
    databaseTooLarge: '数据库文件不能超过 256 MB。',
    updatePrices: '更新单价',
    updatingPrices: '正在更新单价',
    priceUpdateFailed: '单价更新失败：{error}',
    priceUpdateSuccess: 'LiteLLM 与 Cursor 单价已更新',
    priceUpdateWarnings: '价格来源已检查；{count} 条提醒',
    importCursor: '导入 Cursor CSV',
    importingCursor: '正在导入…',
    importFailed: '导入 Cursor CSV 失败：{error}',
    importSuccess: 'Cursor 新增 {imported} 条、更新费用 {updated} 条；重复 {duplicates} 条，跳过 {skipped} 条。',
    importWarnings: '{n} 条 CSV 行警告',
    exportImage: '导出图片',
    exporting: '导出中…',
    exportFailed: '导出图片失败：{error}',
    eyebrow: 'Token 用量与成本报告',
    heroRange: '{since} – {until}',
    heroAsOf: '统计至 {date}',
    heroAgents: '{n} 个 agent',
    heroAgentsOne: '{n} 个 agent',
    heroModels: '{n} 个模型',
    heroModelsOne: '{n} 个模型',
    kpiTokens: 'Tokens',
    kpiTokensSub: '输入 {input} · 输出 {output}',
    kpiCost: '参考费用',
    kpiCostRate: '每百万 tokens {rate}',
    kpiCostUnpriced: '{n} 个模型未定价',
    kpiCache: '缓存命中率',
    kpiCacheSub: '缓存读取 {tokens}',
    kpiRequests: '请求',
    kpiRequestsSub: '{n} 个会话',
    kpiRequestsSubOne: '{n} 个会话',
    dragHandle: '拖动调整卡片顺序（也可用 ↑ ↓ 键）',
    cardHeat: '活动热力图',
    cardAgents: 'Agent 与模型',
    cardModels: '模型用量',
    agentModels: '{agent} 的模型',
    subHeat: '{period} · 每日 Tokens 与费用 · 颜色按 Tokens',
    subAgentsTokens: '{period} · 按 Tokens 排序',
    subAgentsCost: '{period} · 按费用排序',
    colAgent: 'Agent',
    colModel: '模型',
    colTokens: 'Tokens',
    colCost: '费用',
    colMix: 'Token 构成',
    colCache: '缓存命中',
    colRequests: '请求',
    agentHint: '点击 Agent 展开模型 · 悬停查看明细',
    modelHint: '悬停查看明细',
    partialNote: '* 部分请求未定价',
    statActive: '活跃天数',
    statActiveValue: '{n} / {total} 天',
    statAverage: '活跃日均',
    statPeak: '峰值日',
    statStreak: '最长连续',
    statStreakValue: '{n} 天',
    statStreakValueOne: '{n} 天',
    heatLess: '少',
    heatMore: '多',
    heatAria: '{period}每日热力图',
    tipTokens: 'Tokens',
    tipCost: '费用',
    tipRequests: '请求 / 会话',
    tipCache: '缓存命中',
    tipIdle: '无活动',
    tipFuture: '尚未到来',
    tipRates: '右列为 {source} 单价（每百万 tokens）',
    sourceBuiltin: '内置',
    sourceUser: 'pricing.json',
    compInput: '输入',
    compCacheRead: '缓存读',
    compCacheWrite: '缓存写',
    compOutput: '输出',
    rowRequests: '{n} 请求',
    rowRequestsOne: '{n} 请求',
    rowModels: '{n} 个模型',
    rowModelsOne: '{n} 个模型',
    rowUnpriced: '未定价',
    rowPartial: '部分未定价',
    others: '其他 {n} 项模型用量',
    emptyPeriod: '这个周期没有记录。',
    loading: '正在读取本地数据库…',
    errorTitle: '无法加载数据',
    errorBody: '`/api/data` 请求失败：{error}',
    errorHint: '请通过 `toksight web` 打开本页面；开发时运行 `npm run web:dev`。',
    retry: '重试',
    emptyTitle: '还没有任何会话记录',
    emptyBody: '先用 AI agent 跑几个会话，或在报告操作区导入 Cursor 用量 CSV。终端运行 `toksight env` 可以查看各 agent 的扫描位置。',
    warnings: '{n} 条采集警告',
    toastClose: '关闭提示',
    toastDetails: '查看详情',
    footGenerated: '生成于 {time}',
    footRefreshed: '数据刷新于 {time}',
    footPriceUpdated: '{source} 单价更新于 {time}',
    footTimezone: '时区 {tz}',
    footEstimate: '费用按公开价格估算，仅供参考',
    footEstimateCursor: 'Cursor Included 按官网模型单价估算（可能来自缓存），并非实际账单金额',
    footEstimateCursorUnpriced: '费用为参考估算；部分 Cursor 用量未定价',
    footCursorSource: 'Cursor 官方单价',
    footLocal: '数据只来自本机会话文件与导入的 CSV',
    footUnpriced: '未定价模型：{models}',
    dayHint: '双击卡片展开，查看每日详情',
    sheetOpen: '展开卡片',
    sheetClose: '收起卡片（Esc 或 -）',
    sheetHint: '点击日期切换 · ← → 逐日 · Esc 收起',
    deckPrev: '上一页',
    deckNext: '下一页',
    deckPage: '第 {n} 页',
    dayPrev: '前一天',
    dayNext: '后一天',
    dayToday: '今天',
    dayEmpty: '这一天没有记录。',
    secHourly: '时段分布',
    secSessions: '会话',
    hourlyPeak: '峰值 {hour}',
    hourlyAria: '{day}每小时分布',
    sessionsAll: '按 Tokens 排序 · 共 {n} 个',
    sessionsAllOne: '按 Tokens 排序 · 共 {n} 个',
    sessionsTop: '按 Tokens 排序 · 前 {shown} 个 / 共 {n} 个',
    sessionActive: '活跃 {time}',
    sessionUntitled: '未命名会话',
  },
  en: {
    docTitle: 'toksight · Token usage & cost report',
    toolbarAria: 'Report settings',
    modeGroup: 'Period',
    modeMonth: 'Month',
    modeYear: 'Year',
    prev: 'Previous period',
    next: 'Next period',
    themeGroup: 'Theme',
    themeLight: 'Light',
    themeDark: 'Dark',
    themeSystem: 'System',
    langGroup: 'Language',
    refresh: 'Refresh database',
    refreshing: 'Refreshing database',
    refreshFailed: 'Database refresh failed: {error}',
    reportActions: 'Report actions',
    databaseActions: 'All usage data',
    importDatabase: 'Import database',
    exportDatabase: 'Export database',
    importingDatabase: 'Merging…',
    exportingDatabase: 'Backing up…',
    databaseMergeHint: 'Merge duplicates · Keep existing',
    databaseAllHint: 'All agents · All dates',
    databaseImportDescription: 'Merge usage from this backup and skip duplicate records. Existing data and known charges take priority. Imported history remains after a refresh.',
    databaseContents: 'Includes all usage dates, session titles and directories, Cursor imports, and pricing. Accepts toksight databases up to 256 MB.',
    databaseCancel: 'Cancel',
    databaseMerge: 'Merge and import',
    databaseImportSuccess: 'Merged: {imported} added, {updated} charges completed, {duplicates} duplicates.',
    databaseImportFailed: 'Database import failed: {error}',
    databaseExportSuccess: 'Full database backup download started',
    databaseExportFailed: 'Database export failed: {error}',
    databaseInvalid: 'This is not a valid toksight database, or its database version is unsupported.',
    databaseTooLarge: 'Database files must be 256 MB or smaller.',
    updatePrices: 'Update prices',
    updatingPrices: 'Updating prices',
    priceUpdateFailed: 'Price update failed: {error}',
    priceUpdateSuccess: 'LiteLLM and Cursor prices updated',
    priceUpdateWarnings: 'Price sources checked; {count} notices',
    importCursor: 'Import Cursor CSV',
    importingCursor: 'Importing…',
    importFailed: 'Cursor CSV import failed: {error}',
    importSuccess: 'Added {imported} Cursor rows, updated {updated} charges; {duplicates} duplicates and {skipped} skipped.',
    importWarnings: '{n} CSV row warnings',
    exportImage: 'Export image',
    exporting: 'Exporting…',
    exportFailed: 'Image export failed: {error}',
    eyebrow: 'Token usage & cost report',
    heroRange: '{since} – {until}',
    heroAsOf: 'as of {date}',
    heroAgents: '{n} agents',
    heroAgentsOne: '{n} agent',
    heroModels: '{n} models',
    heroModelsOne: '{n} model',
    kpiTokens: 'Tokens',
    kpiTokensSub: 'input {input} · output {output}',
    kpiCost: 'Estimated cost',
    kpiCostRate: '{rate} per million tokens',
    kpiCostUnpriced: '{n} models unpriced',
    kpiCache: 'Cache hit rate',
    kpiCacheSub: '{tokens} cache reads',
    kpiRequests: 'Requests',
    kpiRequestsSub: '{n} sessions',
    kpiRequestsSubOne: '{n} session',
    dragHandle: 'Drag to reorder cards (or use ↑ ↓ keys)',
    cardHeat: 'Activity heatmap',
    cardAgents: 'Agents & models',
    cardModels: 'Model usage',
    agentModels: 'Models for {agent}',
    subHeat: '{period} · daily tokens and cost · shaded by tokens',
    subAgentsTokens: '{period} · sorted by tokens',
    subAgentsCost: '{period} · sorted by cost',
    colAgent: 'Agent',
    colModel: 'Model',
    colTokens: 'Tokens',
    colCost: 'Cost',
    colMix: 'Token mix',
    colCache: 'Cache hit',
    colRequests: 'Requests',
    agentHint: 'Click an agent for its models · hover for details',
    modelHint: 'Hover for details',
    partialNote: '* some requests unpriced',
    statActive: 'Active days',
    statActiveValue: '{n} / {total} days',
    statAverage: 'Per active day',
    statPeak: 'Peak day',
    statStreak: 'Longest streak',
    statStreakValue: '{n} days',
    statStreakValueOne: '{n} day',
    heatLess: 'Less',
    heatMore: 'More',
    heatAria: 'Daily heatmap for {period}',
    tipTokens: 'Tokens',
    tipCost: 'Cost',
    tipRequests: 'Requests / sessions',
    tipCache: 'Cache hit',
    tipIdle: 'No activity',
    tipFuture: 'Not yet',
    tipRates: 'Right column: {source} USD per million tokens',
    sourceBuiltin: 'built-in',
    sourceUser: 'pricing.json',
    compInput: 'Input',
    compCacheRead: 'Cache read',
    compCacheWrite: 'Cache write',
    compOutput: 'Output',
    rowRequests: '{n} requests',
    rowRequestsOne: '{n} request',
    rowModels: '{n} models',
    rowModelsOne: '{n} model',
    rowUnpriced: 'unpriced',
    rowPartial: 'partly unpriced',
    others: '{n} other model uses',
    emptyPeriod: 'Nothing recorded in this period.',
    loading: 'Reading local database…',
    errorTitle: 'Could not load data',
    errorBody: '`/api/data` request failed: {error}',
    errorHint: 'Open this page through `toksight web`; for development run `npm run web:dev`.',
    retry: 'Retry',
    emptyTitle: 'No sessions recorded yet',
    emptyBody: 'Run a few sessions with your AI agents or import a Cursor usage CSV from Report actions. `toksight env` shows where each agent is scanned.',
    warnings: '{n} collection warnings',
    toastClose: 'Dismiss',
    toastDetails: 'Show details',
    footGenerated: 'Generated {time}',
    footRefreshed: 'Data refreshed {time}',
    footPriceUpdated: '{source} prices updated {time}',
    footTimezone: 'Timezone {tz}',
    footEstimate: 'Costs are estimates from public prices',
    footEstimateCursor: 'Cursor Included uses published model rates (possibly cached); this is not a billed amount',
    footEstimateCursorUnpriced: 'Costs are estimates; some Cursor usage remains unpriced',
    footCursorSource: 'Cursor model rates',
    footLocal: 'Data comes from local sessions and imported CSVs',
    footUnpriced: 'Unpriced models: {models}',
    dayHint: 'Double-click the card for daily details',
    sheetOpen: 'Expand card',
    sheetClose: 'Collapse card (Esc or -)',
    sheetHint: 'Click a day to switch · ← → step days · Esc to collapse',
    deckPrev: 'Previous page',
    deckNext: 'Next page',
    deckPage: 'Page {n}',
    dayPrev: 'Previous day',
    dayNext: 'Next day',
    dayToday: 'Today',
    dayEmpty: 'Nothing recorded on this day.',
    secHourly: 'By hour',
    secSessions: 'Sessions',
    hourlyPeak: 'peak {hour}',
    hourlyAria: 'Hourly breakdown for {day}',
    sessionsAll: 'by tokens · {n} sessions',
    sessionsAllOne: 'by tokens · {n} session',
    sessionsTop: 'by tokens · top {shown} of {n}',
    sessionActive: '{time} active',
    sessionUntitled: 'Untitled session',
  },
};

// A `<key>One` entry, when present, is used for `n === 1`.
export function t(locale, key, vars) {
  const table = tables[locale] ?? tables[DEFAULT_LOCALE];
  const one = vars && String(vars.n) === '1' ? table[`${key}One`] : undefined;
  let str = one ?? table[key] ?? tables[DEFAULT_LOCALE][key] ?? tables.en[key] ?? key;
  if (vars) {
    for (const [k, v] of Object.entries(vars)) str = str.replaceAll(`{${k}}`, String(v));
  }
  return str;
}

export function periodLabel(locale, { mode, year, month }) {
  if (mode === 'year') return locale === 'en' ? String(year) : `${year} 年`;
  return locale === 'en' ? `${MONTHS_LONG_EN[month - 1]} ${year}` : `${year} 年 ${month} 月`;
}

// 'YYYY-MM-DD' → "9月12日" / "Sep 12" (with the year when asked).
export function dayLabel(locale, key, withYear = false) {
  const [y, m, d] = key.split('-').map(Number);
  if (locale === 'en') return `${MONTHS.en[m - 1]} ${d}${withYear ? `, ${y}` : ''}`;
  return `${withYear ? `${y}年` : ''}${m}月${d}日`;
}

// 'YYYY-MM-DD' → "周六" / "Sat".
export function weekdayLabel(locale, key) {
  const name = WEEKDAYS[locale === 'en' ? 'en' : 'zh-CN'][weekdayIndex(key)];
  return locale === 'en' ? name : `周${name}`;
}

// Rounded to whole minutes: "1 小时 20 分" / "1h 20m"; "<1m" below a minute.
export function durationLabel(locale, ms) {
  const minutes = Math.round((Number(ms) || 0) / 60000);
  if (minutes < 1) return locale === 'en' ? '<1m' : '不到 1 分钟';
  const h = Math.floor(minutes / 60);
  const m = minutes % 60;
  if (locale === 'en') return h ? `${h}h${m ? ` ${m}m` : ''}` : `${m}m`;
  return h ? `${h} 小时${m ? ` ${m} 分` : ''}` : `${m} 分钟`;
}
