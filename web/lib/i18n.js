...
export const tables = {
  'zh-CN': {
    ...
    emptyBody: '先用 AI agent 跑几个会话，或在上方导入 Cursor 用量 CSV。终端运行 `toksight env` 可以查看各 agent 的扫描位置。',
    warnings: '{n} 条采集警告',
    footGenerated: '生成于 {time}',
    footRefreshed: '数据刷新于 {time}',
    footTimezone: '时区 {tz}',
    footEstimate: '费用按公开价格估算，仅供参考',
    footEstimateCursor: '费用为参考估算；Cursor 套餐内费用未计入',
  ...
  en: {
    ...
    emptyBody: 'Run a few sessions with your AI agents or import a Cursor usage CSV above. `toksight env` shows where each agent is scanned.',
    ...
    footEstimateCursor: 'Costs are estimates; Cursor included usage has no listed charge',
