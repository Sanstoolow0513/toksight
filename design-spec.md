...
> 见 §2 Dark mode）。**仪表盘按板块分标签页**：文字标签栏
> （历史 / 成本 / 会话，accent 下划线，`?tab=` 深链）——历史 = 趋势 + 热力图 + 使用模式；
> 成本 = 周期对比+成本明细 + 用量构成；会话 = 会话表。「只出浅色」的硬约束自此废止，
> 其余纪律全部保留。

> v8.1（2026-09）：KPI 四合一条带并入「Token 活动」热力图卡——四个统计格
> （累计 Tokens / 参考费用 / 缓存命中率 / 活跃天数）作为卡首统计行，格间发丝线分隔，
> 行底发丝线与热力图分节；独立 KPI 条带移除，仪表盘卡片合并为 6 张，
> 标签栏直接位于警告/提示条之下。
...
4. **板块标签栏** `.dash-tabs`（警告/提示条之下）：文字标签 + 底部发丝线，当前标签 accent
   字 + accent 下划线——**历史**（趋势、Token 活动、使用模式）｜**成本**（周期对比+成本明细、
   ...
   **卡片流**（各标签页内自上而下，全部整宽 `.cell`，标题 14px Sans 600）：
   ...
   - 趋势（历史）：范围分段（7/30/90）× 维度分段（构成/Agent）+ 可点击图例（至少留一）
     ...
     实色带（见 §4）。
   - 活动热力图（历史，「Token 活动」）：卡首为四格统计行 `.kpis-inline`（累计 Tokens
     （**accent 值**，副行请求·会话）、参考费用（副行定价覆盖）、缓存命中率（**缓存绿值**，
     副行缓存读 tokens）、活跃天数（副行起始日期），值 28px mono 600，格间发丝线，
     行底发丝线与热力图分节；≤900px 2×2）；下为 GitHub 风格 53 周、方格 3px 圆角、
     2px 缝、paper→ink ramp、少/多图例。
...
5. **页脚**：muted 11px mono：时区、统计范围、生成时间、未定价模型、版本与 local-first 声明。
6. 命中率统计按每次请求归因（session 切模型会被拆分归入各模型，不误计）。最长会话按
...
- **Loading**：首屏骨架（`.skel` 脉冲，panel-2 底）对齐趋势卡 + Token 活动卡（统计行 +
  热力图）形状；刷新不退回
