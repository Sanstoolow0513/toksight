...
export function buildCostCoverage(entries, { pricing, reportedCosts }) {
  const sources = Object.fromEntries(['reported', 'user', 'litellm', 'builtin', 'unknown'].map((id) => [id, { requests: 0, costUsd: 0 }]));
  let unpricedRequests = 0, cacheFallbackRequests = 0;
  const prices = new Map();
  for (const entry of entries) {
    if (entry.costUsd == null) { unpricedRequests++; continue; }
    if (!prices.has(entry.model)) prices.set(entry.model, pricing.priceFor?.(entry.model));
