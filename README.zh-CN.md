...
仪表盘为极简文档风纸面工作表（v8）：暖纸底、白卡片、发丝线分隔，阅读排版用 Geist Sans、
数据排版用 Geist Mono；视觉施工图唯一以 `design-spec.md` 为准。明暗双主题默认跟随系统，
顶栏可手动覆盖（跟随系统 / 浅色 / 深色，存于 `localStorage` 键 `toksight-theme`）。粘性页眉
（mono logo + 上次抓取时间）之后是文字标签栏，把卡片分成三个板块——
**历史**（趋势（先方向后细节）、卡首带四格统计行的 Token 活动热力图、三栏使用模式（按小时｜按月｜活跃节奏））、
**成本**（周期比较（内嵌费用说明）、双栏用量构成（Agent｜模型））、**会话**（会话用量表），
当前板块可用 `?tab=` 深链。卡片高度随内容。页面没有全局筛选栏——时段与维度切换收在
趋势卡内，下方 API 查询参数仍可作为手动 URL 深链使用。悬停是安静的 150ms 淡过渡，
动效全部遵守 `prefers-reduced-motion`。包含：

- **统计行** — 并入「Token 活动」卡卡首，四格发丝线分隔：累计 tokens（赭橙，附请求 · 会话）、
  参考费用、缓存命中率、活跃天数。
- **趋势卡** — 7 / 30 / 90 天窗口 × 两种堆叠（按 token 构成或按 Agent）的按日阶梯实心带；
  图例可点选隐藏序列；卡头右侧放今日 / 近 7 天 / 近 30 天 / 本月汇总标签。时段与 Agent
  视角的切换都在这里。
- **手动深链** — 在页面 URL 中给出 `?period=…&since=…&until=…&client=…` 可重新开窗整个
  仪表盘；超过 366 天时图表显示最后 366 天并注明，汇总和比较仍覆盖完整选择；服务端日期
  按本机时区计算，非法日期（如 2 月 30 日）会被拒绝。
...
- **活动热力图** — GitHub 风格近 53 周每日 tokens 网格（纸面到墨色单色 ramp），悬停看单日明细。
- **用量构成** — 一张双栏卡：左侧各 Agent 的 tokens/费用份额与命中率（点击行展开分模型
  命中率），右侧跨 Agent 汇总的模型排行（每根条把缓存读取（绿色）与新流量硬分两段，
  Agent × 模型明细表折叠保留）。
- **使用模式** — 一张三栏卡：按小时｜按月｜活跃节奏——tokens 在时段与月份上的分布、
  连续活跃、峰值日、按*活跃时长*计的最长会话。
- **会话用量表** — 按 tokens 排名前 10（标题、tokens、请求、命中率、费用、开始时间、活跃时长），
  其余（API 提供前 20）可展开查看。

启动参数（`--client`、`--since`、`--until`、`--today/--week/--month`）限定服务的可见
范围，URL 查询参数只能在其内进一步缩小，去掉参数不会解除启动限制；若上期落在启动范围
