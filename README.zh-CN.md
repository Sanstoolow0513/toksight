...
仪表盘为流式两级布局：活动卡与趋势卡领衔，其后（Agent 分布、模型用量、按小时/按月
直方图）均为无框区块。动效只作交互反馈（悬停、行展开、切换范围/维度时的图表重放），
`prefers-reduced-motion` 下全部关闭——没有入场动画。包含：
...
- **趋势卡片** — 7 / 30 / 90 天窗口 × 两种堆叠维度交叉统计：按 token 构成（新输入 / 缓存
  ...
  汇总胶囊取代了旧的时间范围表
- **Agent 分布区块** — 各 Agent 份额条（tokens、费用、占比）与缓存命中率，点击行展开
  该 Agent 的分模型命中率
- **模型用量区块** — 跨 Agent 汇总的模型排行；每根条内以绿色分段呈现缓存读取占比，
