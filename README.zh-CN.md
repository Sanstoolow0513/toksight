...
仪表盘为极简文档风纸面工作表（v7）：暖纸底、白卡片、发丝线分隔，阅读排版用 Geist Sans、
数据排版用 Geist Mono；视觉施工图唯一以 `design-spec.md` 为准。粘性页眉（mono logo +
上次抓取时间）之后是筛选栏、4 张 KPI 卡、费用说明与周期比较，再是 12 列工作表：趋势
（先方向后细节）、活动热力图、Agent/模型、按小时/按月/活跃节奏、会话用量表。宽屏下每张
卡片都可以拖拽换位、调整大小（布局保存在本地，顶栏可一键重置）；窄屏回退为单列纵向流。
悬停是安静的 150ms 淡过渡，动效全部遵守 `prefers-reduced-motion`。包含：

- **网页筛选** — 全部数据 / 今天 / 近 7 天 / 近 30 天 / 本月 / 自定义日期，以及 Agent
  选择；应用后汇总、图表、模型和会话同步更新，筛选保存在页面 URL 中，刷新后保留。
- **KPI 条** — 累计 tokens（赭橙，附请求 · 会话）、参考费用、缓存命中率、活跃天数。
