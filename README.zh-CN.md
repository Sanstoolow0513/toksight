...
本期 tokens、参考费用、缓存命中率与请求数开始，下面是两张章节卡片：

1. **活动热力图** — 月视图是日历、年视图是 53 周网格，每格按 tokens 或费用着色（卡片内
   切换），并给出活跃天数、活跃日均、峰值日、最长连续与单日悬停明细。
2. **Agent 与模型** — 各 Agent 占本期 tokens 或费用的份额；tokens 模式下长条按输入 / 缓存读 /
   缓存写 / 输出分段，下方附费用、缓存命中率、请求与会话数。每个 Agent 下面按费用列出其模型
   （名次、等宽名称、占本期费用的份额；各原始 ID 费率一致时附单价）。`opus5.5-high` 等 Cursor
   effort 后缀不再出现在名称中。单个 Agent 超过八个模型时，尾部合并为“其他 N 项模型用量”。

点击热力图上的任意一天，报告旁会展开一张**单日详情卡片**：当天的 tokens、参考费用、缓存命中率与
请求数，24 小时时段分布，同样按 Agent 嵌套的模型费用，以及当天的会话（标题、起止时间、活跃时长、
...
指标、配色与语言都记在 `localStorage` 里。**导出图片**会把 KPI 概览、按当前顺序排列的两张卡片与
